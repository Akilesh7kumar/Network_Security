...
class DataValidation:
    ...
    def validate_number_of_columns(self,dataframe:pd.DataFrame)->bool: 
        try:
            ...
            logging.info(f'The DF no of columns {len(dataframe.columns)}')
    ...
    def initiate_data_validation(self)->DataIngestionArtifact:
        try:
            ...
            if not status:
                error_message = f"Train DF does not contain all required columns"
                status = self.validate_number_of_columns(dataframe = train_df)
            status = self.validate_number_of_columns(dataframe = test_df)
            if not status:
                error_message = f"Test DF does not contain all required columns"
            status = self.detect_dataset_drift(base_df=train_df,current_df=test_df)
            dir_path = os.path.dirname(self.data_validation_config.valid_train_file_path)
            os.makedirs(dir_path,exist_ok=True)
            train_df.to_csv(self.data_validation_config.valid_train_file_path,index=False,header=True)
            test_df.to_csv(self.data_validation_config.valid_test_file_path,index=False,header=True)
            data_valiadation_artifact = DataValidationArtifact(
            ...
            return data_valiadation_artifact
        except Exception as e:
            raise NetworkSecurityException(e,sys)
